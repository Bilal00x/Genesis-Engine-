...
class MeshyProvider(BaseProvider):
    ...
    def __init__(self, api_key: Optional[str] = None):
        super().__init__(api_key or os.getenv("MESHY_API_KEY"))
        self.base_url = "https://api.meshy.ai/v1"
        
    async def generate_image(self, prompt: str, **kwargs) -> Dict[str, Any]:
        """Generate image from text prompt using Meshy"""
        # Meshy is primarily for 3D, but can generate images
        ...
        data = {
            "prompt": prompt,
            "model": kwargs.get("model", "meshy-1"),
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, headers=headers, json=data) as response:
                if response.status != 200:
                    ...
                    raise Exception(f"Meshy API error: {response.status} - {error_text}")
    ...
    async def generate_3d(self, prompt: str, **kwargs) -> Dict[str, Any]:
        ...
        data = {
            "prompt": prompt,
            "model": kwargs.get("model", "meshy-3d"),
    ...
    async def generate_animation(self, model_url: str, **kwargs) -> Dict[str, Any]:
        ...
        # Meshy can generate animations from 3D models
    ...
    async def generate_audio(self, prompt: str, **kwargs) -> Dict[str, Any]:
        ...
        # Meshy doesn't have audio generation
        raise NotImplementedError("Meshy provider doesn't support audio generation")
    
    async def generate_code(self, prompt: str, **kwargs) -> Dict[str, Any]:
        """Generate code from text prompt"""
        # Meshy doesn't have code generation
        raise NotImplementedError("Meshy provider doesn't support code generation")